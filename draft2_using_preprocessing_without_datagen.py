...
with mlflow.start_run():
    ...
    history = model.fit(
        x=np.array(train_images), y=train_labels,  # Convert to numpy array
        batch_size=32,
        epochs=1,
        validation_data=(np.array(val_images), val_labels),  # Convert to numpy array
...
# Evaluate the performance of the model
    ...
    evaluation = model.evaluate(
        x=np.array(test_images), y=test_labels, verbose=1, batch_size=32
    )
